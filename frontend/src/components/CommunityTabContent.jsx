...
const CommunityTabContent = ({ user }) => {
  ...
  const fetchSharedMeditations = async () => {
    try {
      ...
      if (response.data && response.data.meditations) {
        ...
        // Set liked meditations based on userLiked field
        if (user) {
          const likedIds = response.data.meditations
            .filter(med => med.userLiked)
            .map(med => med._id);
          setLikedMeditations(new Set(likedIds));
        }
      }
    } catch (error) {
  ...
  const handleLikeMeditation = async (meditationId) => {
    ...
    try {
      ...
      const endpoint = API_ENDPOINTS.COMMUNITY_LIKE(meditationId);
      ...
      setCommunityMeditations(prev => prev.map(med => 
        med._id === meditationId 
          ? { ...med, likeCount: (med.likeCount || med.likes || 0) + (isLiked ? -1 : 1) }
          : med
      ));
    } catch (error) {
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '12px' : '16px', marginBottom: '12px' }}>
              ...
              <div style={{
                ...
                backgroundImage: meditation.customImage ? `url(/assets/images/shared/${meditation.customImage.filename})` : 'none',
                backgroundSize: 'cover',
                backgroundPosition: 'center'
              }}>
                {!meditation.customImage && (meditationTypes.find(t => t.value === meditation.meditationType)?.emoji || '🧘')}
              </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{
                  ...
                  <span>•</span>
                  <span>{meditation.likeCount || meditation.likes || 0} likes</span>
                  <span>•</span>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {meditation.audioFile && (
                <div style={{ flex: 1, minWidth: '200px' }}>
                  ...
                  >
                    <source src={`/assets/audio/shared/${meditation.audioFile.filename}`} type="audio/mpeg" />
